...
public class BanManager {
     ...
     * @param uuid : Banned player's uuid
     * @param endInSeconds : End of the ban in seconds (if -1 = permanent)
     * @param reason : The ban's reason
     */
    public void ban(String author, UUID uuid, long endInSeconds, String reason){
        ...
        if(banSystem.getServer().getPlayer(uuid).isPresent()){
            ...
            target.disconnect(PlaceHolderParser.parseBanPlaceHolders(Configs.MESSAGE_CONFIG.BAN_KICKED, this, target));
        }
    }
    ...
    public void unban(UUID uuid){
        ...
        mongoUtil.getBanned().deleteOne(new Document("uuid", uuid.toString()));
    }

    public boolean isBanned(UUID uuid){
        return mongoUtil.getBanned().countDocuments(new Document("uuid", uuid.toString())) > 0;
    }
    ...
    private Document getDocument(UUID uuid){
        return mongoUtil.getBanned().find(new Document("uuid", uuid.toString())).limit(1).cursor().next();
    }
